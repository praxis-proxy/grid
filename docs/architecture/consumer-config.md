# Operator-Owned Consumer Config

This document describes the target design for generating consumer gateway
configuration from Grid state.  The goal is to make the Grid operator the source
of truth for routing config while keeping provider credential values out of Grid
overlays, statuses, logs, route metadata, and gateway ConfigMaps.

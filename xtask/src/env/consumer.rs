...
//! defined in the environment config.  Two `intelligent_route` config modes are
//! supported:
//!
//! - **Static mode (default):** Generates candidates from provider site/model declarations in the environment config.
//! - **Overlay mode:** Reads a routing overlay `routing-config.json`. The overlay `local_site` and candidates drive
//!   `intelligent_route`; the `load_balancer` section is still generated from provider endpoints in the environment
//!   config.
//!
...
/// generated certificates, applies a `intelligent_route` `ConfigMap`, and
/// deploys the Praxis AI gateway Deployment and Service.
///
/// When `overlay_config` is `Some(path)`, reads the routing overlay
/// `routing-config.json` at that path. The overlay `local_site` and candidates
...
/// Deploy the consumer gateway with `openai_responses_format` → `intelligent_route` filter chain.
///
...
/// names.  The `intelligent_route` candidates use `gateway-{candidate.site}`.  For
...
/// `openai_responses_format` → `intelligent_route` → `load_balancer`.
...
///
/// The `intelligent_route` cluster convention is `gateway-{site}`, which must match
...
fn consumer_gateway_config(
    ...
    let candidates = if let Some(o) = overlay {
        ...
        build_intelligent_route_candidates(providers)
    };
...
/// Consumer gateway config with `openai_responses_format` → `intelligent_route` filter chain.
///
...
///
/// `intelligent_route` reads the model from `X-Model` and selects from the Grid overlay
...
/// Build `intelligent_route` candidate entries from provider endpoints.
///
...
fn build_intelligent_route_candidates(providers: &[ProviderEndpoint]) -> String {
    providers
        .iter()
        .flat_map(|ep| {
...
///
/// This allows `credential_inject` to use `file:` as its token source
...
///
/// Generates a `intelligent_route` section from the overlay (which must include both
...
/// or in the `intelligent_route` candidates block.
#[expect(
    clippy::too_many_lines,
...
/// operator overlay JSON, or in the `intelligent_route` candidates block.
#[expect(
    dead_code,
...
///
/// All four backend clusters are included so the `intelligent_route` candidates from
...
/// promotes it to the `X-Model` header for `intelligent_route`.
...
mod tests {
    ...
    fn static_candidates_include_all_provider_models() {
        ...
        assert!(
            ...
            "first model must appear in intelligent_route candidates"
        );
        assert!(
            config.contains("model-2"),
            "second model must appear in intelligent_route candidates"
        );
    }
    ...
    #[test]
    fn intelligent_route_and_load_balancer_filters_present() {
        let providers = vec![make_provider("site-a", &["model"])];
        let config = consumer_gateway_config("consumer", &providers, None);
        assert!(
            config.contains("filter: intelligent_route"),
            "intelligent_route filter must be present"
        );
        assert!(
    ...
    #[test]
    fn responses_config_uses_responses_format_before_intelligent_route() {
        let providers = vec![make_provider("site-a", &["model"])];
        ...
        let route_pos = config.find("filter: intelligent_route");
        assert!(
            responses_pos.is_some(),
            "responses config must include openai_responses_format"
        );
        assert!(route_pos.is_some(), "responses config must include intelligent_route");
        assert!(
            matches!((responses_pos, route_pos), (Some(responses), Some(route)) if responses < route),
            "openai_responses_format must run before intelligent_route"
        );
        assert!(
            config.contains("model: X-Model") && config.contains("model_header: X-Model"),
            "responses format and intelligent_route must agree on the model header"
        );
        assert!(
            !config.contains("filter: credential_inject"),
    ...
    fn candidate_cluster_matches_load_balancer_cluster() {
        let providers = vec![make_provider("site-a", &["model"])];
        let config = consumer_gateway_config("consumer", &providers, None);
        // Both intelligent_route candidate and load_balancer must use gateway-{site}
        let candidate_marker = "cluster: gateway-site-a";
        let cluster_marker = "- name: gateway-site-a";
        assert!(
            config.contains(candidate_marker),
            "intelligent_route candidate must reference gateway-site-a"
        );
        assert!(
    ...
    fn overlay_mode_uses_consumer_site_as_local_site() {
        ...
        assert!(
            ...
            "GridNetwork name must not appear as intelligent_route local_site"
        );
    }
    ...
    #[test]
    fn both_modes_include_intelligent_route_filter() {
        let providers = vec![make_provider("site-a", &["model"])];
        ...
        assert!(
            static_config.contains("filter: intelligent_route"),
            "static mode must include intelligent_route"
        );
        assert!(
            overlay_config.contains("filter: intelligent_route"),
    ...
    // -----------------------------------------------------------------------
    // Native credential injection (credential_inject)
    // -----------------------------------------------------------------------
    ...
    #[test]
    fn native_config_uses_credential_inject_filter() {
        let overlay = make_test_overlay_with_credential();
        let providers = make_test_providers();
        let config = build_api_fallback_consumer_config_native(
            "consumer",
        ...
        assert!(
            config.contains("filter: credential_inject"),
            "native config must use credential_inject filter"
        );
        assert!(
    ...
    fn native_config_uses_file_source_not_inline_value() {
        ...
        assert!(
            !config.contains("\n            value:"),
            "native config must not use inline value: in credential_inject"
        );
    }
    ...
    #[test]
    fn token_absent_from_intelligent_route_candidates_section() {
    ...
    fn header_injection_api_fallback_config_unchanged_by_native_addition() {
        ...
        assert!(
            !config.contains("credential_inject"),
            "header-injection config must not include credential_inject"
        );
    }
}

...
//!   `grid_ingress_trust` rejects with HTTP 403.
...
/// 4. Negative (filter layer): same trusted CA but wrong organization → HTTP 403 from `grid_ingress_trust`.
///
...
/// Case 4 confirms that `grid_ingress_trust` enforces organization
...
/// organization → HTTP 403 from `grid_ingress_trust`.
///
...
/// `grid_ingress_trust` then checks the organization field and rejects.
///
    ...
    match http_status {
        Some(403) => {
            tally.pass(
                cluster,
                &format!(
                    "same-CA wrong-org client rejected by grid_ingress_trust \
            ...
            tally.fail(
                ...
                &format!(
                    "same-CA wrong-org cert returned HTTP {s} \
                     (expected 403 from grid_ingress_trust)"
                ),
                ctx,
            );
        },
